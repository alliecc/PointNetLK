...
class Action:
    ...
    def create_from_pointnet_features(self, ptnet):
        return ptlk.pointem.PointEM(ptnet, self.delta, self.learn_delta)
    ...
    def compute_loss(self, model, data, device):
        ...
        r = ptlk.pointem.PointEM.do_forward(model, p0, p1, self.max_iter, self.xtol,\
                                            self.p0_zero_mean, self.p1_zero_mean)
        #r = model(p0, p1, self.max_iter)
        est_g = model.g

        loss_g = ptlk.pointem.PointEM.comp(est_g, igt)

        if self._loss_type == 0:
            loss_r = ptlk.pointem.PointEM.rsq(r)
            loss = loss_r
        elif self._loss_type == 1: #default
            loss_r = ptlk.pointem.PointEM.rsq(r)
            loss = loss_r + loss_g
        elif self._loss_type == 2:
            pr = model.prev_r
            if pr is not None:
                loss_r = ptlk.pointem.PointEM.rsq(r - pr)
            else:
                loss_r = ptlk.pointem.PointEM.rsq(r)
            loss = loss_r + loss_g
        else:
